...
**Research supersedes standing rules for the DFY Content Calendar (Decision 111, 24 September 2026).** Every build starts with extremely thorough viral research: a large set of Reels and posts that got a disproportionate views-to-followers ratio, a major engagement jump or a big follower influx, from accounts in the same niche and of a similar creator type that are normally not viral. Never celebrities or unrealistic accounts. The build finds the patterns and replicates them for the calendar in the persona's own words. Whatever that research proves supersedes any standing rule in canon, SOP 15 or any other document, and the build names every override with its proof. The only rules research cannot override: no income or earnings claims on Meta surfaces, no faith content, no em dashes, and no invented products, prices or links. The Tuesday to Thursday and Vault Wednesday posting rules that used to sit on this list were replaced 25 Sep 2026 by the spread rules, Decision 117. The persona is not required in every calendar image; person-free images (her desk, porch, kitchen, hands, a page) are normal and run on Seedream 4.5 with Unlimited on. Process: SOP 15 Phase A, rewritten the same day.
...
**The Vault never appears in the same asset as a membership tier**, never framed as a related product. Frame by question, never by price. Its old adjacent-day and Wednesday-only rules were replaced 25 Sep 2026 by the spread rules, Decision 117.
...
**DFY services (Decision 116, 25 September 2026).** Eleven done-for-you services built on Jodie's own automations, sold from the Open DFY Services classroom course by DM keyword. Each has a row in `products[]`; prices and order words are in `claude/TDIE_DFY_SERVICES.md`. The spicy DFY lesson is Jodie's and is never edited. **Beacons checkout added (Decision 118, 25 September 2026).** Each DFY service also has a Beacons checkout page now, so a buyer can pay directly instead of only DMing — `beacons_checkout_url` or `beacons_checkout_urls` on the product row in `canon.json`. Two exceptions: Run It Like Mine: DFY Automation Setup has no Beacons product for any of its four tiers (DM-only), and the recurring/monthly halves of DFY Viral Instagram Content Calendar and DFY Skool Autopilot (plus the Skool Autopilot Mirror add-on) are DM-only — their one-time/setup prices do have checkout links.
...
**The Skool affiliate rotation.** The weekly cap of three affiliate posts (20 September 2026) was replaced 25 Sep 2026 by the spread rules, Decision 117. Affiliates stay occasional and alternated in: never two affiliate posts on the same day or on consecutive days, never the same affiliate product twice in a week, never in the day's teaching slot, and the disclosure always in the same post. The old per-product cycles (AIM's six posts, The Brand Closet™'s four) still do not run independently; they draw from one shared queue. Selling someone else's product is the exception in the feed, not the drumbeat.
...
- **The posting day rules (no membership tier content Tuesday to Thursday; Pretty & Paid PLR Vault™ Wednesday only; Vault and membership never on the same or adjacent days; Premium only on Monday or Friday; a weekly affiliate cap of three) were replaced 25 Sep 2026 by the spread rules, Decision 117.** Any offer can run any day. Teach first, value second, sell third still holds, and selling is never the majority of a week. At most one selling post a day in the Skool feed (outside a scheduled promo window such as the Kit window), never two selling posts back to back. The same offer never runs two days in a row, and not more than twice in a week unless it is a scheduled promo window. Affiliates stay occasional and alternated in: never two affiliate posts on the same day or on consecutive days, never the same affiliate product twice in a week, never in the 11:00 AM teaching slot, disclosure always in the same post. Nothing repeats: check the last 14 days of sent and scheduled posts and never reuse the same topic angle, hook, opening line, image or freebie inside that window. Spread offers across the week so no stretch of days is all selling and no single offer clusters.
...
- **Links on Meta (Decision 115, 25 September 2026; Instagram part replaced by Decision 117, 25 September 2026).** **Instagram:** No links anywhere on Instagram, ever. Not in a caption, not in a comment, not in a first comment, not in a reply. Links do not work on Instagram. An Instagram call to action points to the link in bio and says in words what they will find there. Instagram Story link stickers are the one exception and stay as they are. **Facebook:** no link of any kind goes in a Facebook post body, because Meta suppresses posts that carry one. The post names the call to action in words and points to the comments ("link's in the comments 👇"). The link goes only in the first comment, set to post automatically 5 minutes after the post's scheduled publish time; where the scheduler cannot do that, the comment goes up by hand 5 minutes after the post is live, and no post is reported done until its first-comment link has been seen live. Facebook comments cannot hide a link behind words, so there the link sits on its own line after a short call-to-action line. **Skool links are never a raw web address:** anywhere a link can sit behind words (Skool posts, comments and lessons, email, the site, PDFs), a Skool link is a hyperlink buried in the call-to-action words.
...
**Governing document for the community feed:** `claude/TDIE_SKOOL_POSTING_SYSTEM.md` — the slot times, the weekly mix and the spread rules. It is what the weekly SkoolKit load reads from.
...
- **Links never go in a Facebook or Instagram post (Decision 115, 25 September 2026).** Founder rule. The link goes in the first comment, 5 minutes after the post publishes, and a Skool link is always hyperlinked words, never a raw URL. Never re-raised, and never read as the "link-in-comment tactic" Decision 107 closed: that decision covers outreach and reply comments left on other people's Threads and Instagram posts, and it still stands. Instagram part superseded by Decision 117: no links anywhere on Instagram, calls to action point to the link in bio.
...
- **DFY services also have Beacons checkout pages (Decision 118, 25 September 2026).** Added alongside the DM-to-order flow; discount terms (15% Premium, 25% for a service's first 2 clients, non-stacking) apply at checkout and in the Skool lesson bodies. Not re-raised as a conflict with Decision 116, which is still correct that there were no Beacons listings for these services *at launch*.
...
**117.** (25 September 2026) **Instagram carries no links, and the posting day rules are replaced by spread rules.** Founder instruction. Instagram: no links anywhere, ever (captions, comments, first comments, replies); calls to action point to the link in bio; Story link stickers excepted. Facebook first-comment rule unchanged. Posting day rules (no membership Tue to Thu, Vault Wednesday only, Vault/membership never adjacent, Premium Monday or Friday only, weekly affiliate cap of three) removed and replaced by spread rules: any offer any day; at most one selling post a day; never two selling posts back to back; same offer never two days running or more than twice a week outside a promo window; affiliates never same or consecutive days, never the same product twice a week; nothing repeats inside 14 days. Reel renders on Higgsfield must cost under 30 credits (Wan 3.0 720p default, Grok Imagine 1.5 fallback). Jodie wrote the day rules when she rarely posted, only so the feed would not be all sales. Recorded the same day in §3, §4, §5, §7, `canon.json` (meta.decision_117_2026_09_25 and content_calendar_rules.spread_rules), `claude/TDIE_SKOOL_POSTING_SYSTEM.md`, `SOP-15-dfy-content-calendar.md` and `claude/TDIE_THREADS_SYSTEM.md`.
---

**118.** (25 September 2026) **Conversion-optimization pass on the 11 DFY services: Beacons checkout, a service picker, curriculum hand-offs and buy blocks in the lesson bodies.** Founder instruction, given after asking "add them to canon. anything else to optimize or expand conversion for dfy services?" — seven actions proposed, all approved ("Please complete all seven"), plus discount terms: Premium members 15% off; a service's first 2 clients 25% off in exchange for a testimonial, not stacking with the Premium discount; Beacons confirmed signed in. Shipped: a Beacons checkout page for each of the 12 fixed-price services (individualized cover image per service, one shared template); a "which service is for me" picker on the How It Works lesson; hand-off lines added to 7 matching teaching lessons elsewhere in the curriculum, covering 8 of the 11 DFY services (Etsy and Amazon skipped — no matching teaching lesson exists to hang one on); a "Ready to order?" buy block with the discount terms and a hyperlinked Beacons buy link added into all 12 DFY lesson bodies themselves, each one read back from the server after writing (Run It Like Mine, which has no Beacons product, got the discount-terms note only, DM-only); and the Beacons checkout URLs added to the `products[]` rows in `canon.json`. Doc of record: `claude/TDIE_DFY_SERVICES.md`.
---
