...
| **Amazon** | **Jodie is an approved Amazon Associate and Amazon Influencer** (Decision 101, 24 September 2026). One Associates account, one tracking tag; the Influencer storefront runs on it. Registered in `canon.json → products[]` as "Amazon Influencer Storefront" (not owned). Storefront: `https://www.amazon.com/shop/influencer-adc3fcaa` (Decision 108). Used on the Legally Blonde Pinterest boards, on Instagram `@itstommykate`, and on the site's `/lifestyle` section. Every link ships with the Amazon disclosure. Never a price. |
...
**Governing document for Threads:** `claude/TDIE_THREADS_SYSTEM.md`, voice companion `claude/TDIE_JODIE_THREADS_VOICE.md` — the schedule, the account, and Jodie's own voice as the account's real, named founder. See Decision 92.
...
- **Leni Loves is $27.** Settled 21 September 2026 by the founder. The $37 and $17 figures are dead. Never re-raised as a pricing conflict.
