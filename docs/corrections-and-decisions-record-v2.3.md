...
# RECORD v2.3 ADDENDUM — 24 July 2026 (email automation go-live prep)

*Appended to Corrections & Decisions Record v2.2. Adds Decisions 77–80.*

# DECISION 77 — RSS AUTO-EMAIL CAMPAIGN IS BUILT (DRAFT, PENDING GO-LIVE)

A MailerLite RSS campaign auto-emails the list when a new article publishes. Configuration:
- **Type:** RSS campaign ("RSS - New Blog Post Notification")
- **Feed:** https://learn.thedigitalincomeedit.com/rss.xml
- **Recipients:** All active subscribers (everyone, regardless of group)
- **Frequency:** Daily check at **13:00 UTC (9 AM Eastern)**, all 7 days
- **"New posts" toggle:** ON — each email includes only posts published since the last send
- **Subject line:** `A New Guide from The Digital Income Edit` (static — see D78)
- **Sender:** The Digital Income Edit · hello@thedigitalincomeedit.com (see D79)
- **Body:** intro line → MailerLite "Basic blog feature" RSS block (auto-fills title/excerpt/image/Read-more from feed at send time) → outro with free-community mention → footer

**Status: DRAFT.** Not scheduled. Go-live requires: DNS verify → sender set → test send confirmed by founder → founder clicks Schedule.

**Known platform limitation (do not try to "fix"):** MailerLite RSS campaigns have no true per-post send — only daily/weekly/monthly digest schedules. At the founder's publishing pace (roughly one article at a time), daily + "New posts" toggle behaves like per-post; only multiple posts within one 24h window batch into a single email. This is the platform ceiling, accepted by decision.

**Publishing dependency:** an article only triggers an email if it is in `src/data/article-dates.js` (the RSS date manifest). Forgetting that line = no email. When Claude publishes, this is automatic; manual publishes must add it.

# DECISION 78 — SUBJECT LINE IS STATIC ("A New Guide from The Digital Income Edit")

MailerLite's editor exposes no RSS merge tag for the subject on this plan, so the article title cannot be auto-inserted into the subject. The subject is a fixed, evergreen line: **A New Guide from The Digital Income Edit**. It reinforces sender recognition (helps deliverability on the new domain). If a future plan supports dynamic RSS subject variables, swap in the article title.

# DECISION 79 — SENDER DOMAIN AUTHENTICATED; SENDER IS hello@thedigitalincomeedit.com

To fix deliverability (Gmail-as-sender was flagged "not recommended"), `thedigitalincomeedit.com` was authenticated in MailerLite. DNS records added at Porkbun:
- **CNAME (DKIM):** `litesrv._domainkey` → `litesrv._domainkey.mlsend.com`
- **TXT (verification):** root → `mailerlite-domain-verification=fa15e3fd4a41382976f6b24f10b7bfc41af3918a`
- **TXT (SPF):** existing root SPF **edited in place** (not duplicated) to merge MailerLite's include. Final value: `v=spf1 include:_spf.mlsend.com include:_spf.porkbun.com ~all` — single SPF record, both includes, `~all` preserved.

All prior DNS (Google TXT, Pinterest TXT, MX fwd1/fwd2.porkbun.com, A records, CNAMEs) confirmed intact. Sender address once verified: **hello@thedigitalincomeedit.com** (domain auth covers any address on the domain).

**Status:** records placed; DNS propagating; verification (MailerLite "Check records") pending.

# DECISION 80 — SUBSCRIBER MODEL, IMPORT GROUP, AND FOOTER ADDRESS

- **One-list model (Option 1) confirmed:** all freebies + newsletter capture into one MailerLite pool; the RSS campaign sends to "All active subscribers." The 12 pillar groups exist but are unused pending future segmentation.
- **Newsletter/freebie form** currently routes signups into the "AI" group — a mislabel (everyone tagged "AI" regardless of pillar). Harmless for the campaign (sends to all), but flagged to fix when per-pillar segmentation is built. Rename or re-point later.
- **"Existing Subscribers" group created** for importing the founder's prior opted-in list. Import is a founder task (opt-in attestation required); not automated.
- **Footer physical address = founder's home address, retained by founder decision.** CAN-SPAM requires a physical address; MailerLite won't send without one. Founder chose to keep the home address rather than obtain a PO Box. **Flagged:** for a faceless brand this is the one seam where private info goes to every subscriber permanently; a PO Box (~$10/mo) is the recommended future upgrade. Not blocking.
